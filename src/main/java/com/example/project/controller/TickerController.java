...
public class TickerController {
    ...
    public String queryshares() {
        ...
        order.setPrice(0d);
        List<TickerEntity> tickers = entityMapper.selectByDate("2023-07-08 09:54:56", "2023-08-08 09:54:56");
...
//                //获取macd
        ...
        for (int i = 1; i < bollTargets.size() + 1; i++) {
            ...
            seed.setMacd(macdList.get(macdList.size() - 1));
            seed.setBoll(bollTargets.get(bollTargets.size() - 1));
            seedList.add(seed);
        }
        ...
        for (int i = 300; i <list.size() ; i++) {
            ...
            if(order.getAmount() != 0){
                ...
                if(order.getDirection().equals("duo")){
                    ...
                    //止盈价格
                    Double zyb = new BigDecimal(price).multiply(new BigDecimal("1.011")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                    //止损价格
                    Double zsb = new BigDecimal(price).multiply(new BigDecimal("0.989")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                    //止盈点数量
                    Double zyCount = new BigDecimal(order.getAmount()).multiply(new BigDecimal("0.66")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                   //止损点数量
                    Double zsCount = new BigDecimal(order.getAmount()).multiply(new BigDecimal("0.98")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                    //先判断是否止损，再判断止盈
                ...
                if(order.getDirection().equals("kong")){
                    ...
                    Double price = order.getPrice();
                    //止盈价格
                    Double zyb = new BigDecimal(price).multiply(new BigDecimal("0.989")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                    //止损价格
                    Double zsb = new BigDecimal(price).multiply(new BigDecimal("1.011")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                    //止盈点数量
                    Double zyCount = new BigDecimal(order.getAmount()).multiply(new BigDecimal("0.66")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                    //止损点数量
                    Double zsCount = new BigDecimal(order.getAmount()).multiply(new BigDecimal("0.98")).setScale(2, BigDecimal.ROUND_HALF_DOWN).doubleValue();
...
//                                Double kongD  = new BigDecimal(account).setScale(6, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                                ...
                                order.setPrice(entity.getSpj());
//                                // 测试文本邮件发送（无附件）
                                String content = "我在=" + time + "的59分59秒=开空了" + "开多价格是=" + entity.getSpj();
                                System.out.println("开始了" + content);
//                                Order order = getTrad(tradeAPIService, accountAPIService, marketDataAPIService,"sell");
                ...
                //通过KDJ下单
                getOrderByKdj(time, entity, kdjNow, kdjPre,bollNow);
    ...
    private void getOrderByKdj(String time, TickerEntity entity, KdjTarget target,KdjTarget targetPre, BollTarget bollNow) {
...
//                    Double kongD  = new BigDecimal(account).setScale(6, BigDecimal.ROUND_HALF_DOWN).doubleValue();
                    ...
                    order.setDirection("kong");
                    order.setPrice(entity.getSpj());
                    String content = "我在b=" + time + "的59分59秒=开空了" + "开多价格是=" + entity.getSpj();
                    System.out.println("开始了" + content);
                    //开仓设置止盈止损成功
                    total = total + 1;
                }
